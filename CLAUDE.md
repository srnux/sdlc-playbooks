...
In Claude Code, the commands in `.claude/commands/` run the gate at expansion time and then
name the playbook: `/design-system`, `/requirements`, `/prototype`, `/approve`, `/build`,
`/status`. The playbooks themselves are projected into `.claude/skills/` from `playbooks/`
by `node .claude/tools/sync.mjs`.
